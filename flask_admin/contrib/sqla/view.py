...
class ModelView(BaseModelView):
    ...
    def get_query(self):
        ...
        """
        return self.session.query(self.model).filter_by(is_show=1, is_del=0)
    ...
    def get_count_query(self):
        ...
        """
        return self.session.query(func.count('*')).select_from(self.model).filter_by(is_show=1, is_del=0)
    ...
    def create_model(self, form):
        ...
        try:
            ...
            if hasattr(model, "title"):
                model.hash = generate_hash(f"{model.title}_{model.update.timestamp()}")
            elif hasattr(model, "name"):
                model.hash = generate_hash(f"{model.name}_{model.update.timestamp()}")
            else:
                model.hash = generate_hash(f"{model.update.timestamp()}")

            if hasattr(model, "title") and hasattr(model, "content"):
                from blog_admin.utils import create_article_file
                article_filepath = create_article_file(model.title, model.content, article_create=model.create)
                model.content_path = article_filepath
